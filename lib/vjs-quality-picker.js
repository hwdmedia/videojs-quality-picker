...
function qualityPickerPlugin(options) {
    ...
    function onQualityData(event, {qualityData, qualitySwitchCallback}) {
        ...
        for (var i=0; i < SUPPORTED_TRACKS.length; i++) {
            ...
            var qualityPickerButton = player.controlBar.getChildById(name);
            ...
            if (qualityData[track] && qualityData[track].length > 1) {
                qualityPickerButton = new QualityPickerButton(player, {id: name, name: name, qualityList: qualityData[track], qualitySwitchCallback, trackType: track});
